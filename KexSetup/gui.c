	...
	IN	PVOID	Parameter)
{
	...
	if (WaitResult == WAIT_TIMEOUT) {
		ErrorBoxF(_(L"The elevated setup process appears to have stopped responding."), ExitCode);
		SendMessage(MainWindow, WM_USER + 3, 0, 0);
	} else if (!NT_SUCCESS(ExitCode)) {
		ErrorBoxF(_(L"The elevated setup process exited with an error code: 0x%08lx"), ExitCode);
		SendMessage(MainWindow, WM_USER + 3, 0, 0);
	} else {
	...
	if (Message == WM_INITDIALOG) {
		...
		if (OperationMode == OperationModeInstall) {
			...
			ToolTip(
				Window,
				IDS4PRESERVECONFIG,
				(PWSTR)_(L"Preserve the application compatibility settings (such as Windows version spoofing) "
		...
		if (ControlId == IDCANCEL2) {
			...
			if (CurrentScene == SCENE_INSTALLATION_COMPLETE ||
				...
				if (ShellExecuteResult <= 32 && ShellExecuteResult != SE_ERR_ACCESSDENIED) {
					ErrorBoxF(_(L"ShellExecute failed with error code %d. Setup cannot continue."), ShellExecuteResult);
