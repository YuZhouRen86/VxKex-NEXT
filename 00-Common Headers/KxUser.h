...
typedef enum _DISPLAY_DEVICE_TYPE{
	DEVICE_PRIMARY		= 0,
	DEVICE_IMMERSIVE	= 1
} TYPEDEF_TYPE_NAME(DISPLAY_DEVICE_TYPE);
...
typedef enum _POINTER_INPUT_TYPE {
	...
	PT_MOUSE,
	PT_TOUCHPAD
} TYPEDEF_TYPE_NAME(POINTER_INPUT_TYPE);
...
typedef INT TOUCH_FLAGS;
typedef INT TOUCH_MASK;
typedef INT PEN_FLAGS;
typedef INT PEN_MASK;
...
typedef struct _POINTER_PEN_INFO {
	POINTER_INFO	pointerInfo;
	PEN_FLAGS		penFlags;
	PEN_MASK		penMask;
	UINT32			pressure;
	...
	INT32			tiltX;
	INT32			tiltY;
} TYPEDEF_TYPE_NAME(POINTER_PEN_INFO);
...
typedef struct _FEATURE_ERROR {
	...
	PCSTR	originName;
} TYPEDEF_TYPE_NAME(FEATURE_ERROR);

typedef enum _FEATURE_CHANGE_TIME {
	FEATURE_CHANGE_TIME_READ = 0,
	FEATURE_CHANGE_TIME_MODULE_RELOAD = 1,
	FEATURE_CHANGE_TIME_SESSION = 2,
	FEATURE_CHANGE_TIME_REBOOT = 3
} TYPEDEF_TYPE_NAME(FEATURE_CHANGE_TIME);

typedef enum _FEATURE_ENABLED_STATE {
	FEATURE_ENABLED_STATE_DEFAULT = 0,
	FEATURE_ENABLED_STATE_DISABLED = 1,
	FEATURE_ENABLED_STATE_ENABLED = 2
} TYPEDEF_TYPE_NAME(FEATURE_ENABLED_STATE);
